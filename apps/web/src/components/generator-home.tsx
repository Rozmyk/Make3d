...
export function GeneratorHome({ initialView = "landing" }: { initialView?: HomeView }) {
  ...
  return <motion.main className="projectsHome" {...enter()}>
    <header className="consoleHeader"><motion.nav className="floatingNav" aria-label="Main navigation" {...enter(0.08)}><button className="brand brandButton" onClick={() => navigate("landing")} aria-label="Make3D home"><img className="brandLogo" src="/make3d-logo.svg" alt="Make3D" /></button><div className="navLinks"><button className={`navLink${isLibraryView ? " is-active" : ""}`} onClick={() => navigate("categories")} aria-current={isLibraryView ? "page" : undefined}>Library</button><a className="navLink" href="https://github.com/Rozmyk/Make3d" target="_blank" rel="noreferrer">GitHub <span aria-hidden="true">↗</span></a><button className={`navLink navLink--contact${view === "contact" ? " is-active" : ""}`} onClick={() => navigate("contact")} aria-current={view === "contact" ? "page" : undefined}>Contact</button></div></motion.nav></header>
    {view === "landing" ? <Landing onBrowse={() => navigate("categories")} onSelect={navigate} onContact={() => navigate("contact")} /> : view === "contact" ? <Contact /> : view === "categories" ? <>
      <motion.section className="projectsCatalogue projectsCatalogue--home" aria-labelledby="categories-title" {...enter(0.15)}>
        ...
        <div className="categoryGrid">
          <motion.button className="categoryCard" onClick={() => navigate("organizers")} {...cardMotion(0)}><CategoryIcon type="organizers" /><strong>Organizers</strong><span className="projectCardDescription">Configurable trays, boxes and compact storage for the little things that need a place.</span><span className="projectCardAction">Browse organizers <span aria-hidden="true">→</span></span></motion.button>
          ...
          <motion.button className="categoryCard" onClick={() => navigate("desk-mounts")} {...cardMotion(2)}><CategoryIcon type="mount" /><strong>Desk mounts</strong><span className="projectCardDescription">Made-to-fit holders for hardware mounted under the desk.</span><span className="projectCardAction">Browse desk mounts <span aria-hidden="true">→</span></span></motion.button>
          <motion.button className="categoryCard" onClick={() => navigate("pegboard")} {...cardMotion(3)}><CategoryIcon type="pegboard" /><strong>Pegboard</strong><span className="projectCardDescription">Modular add-ons for IKEA Skådis-compatible pegboards.</span><span className="projectCardAction">Browse pegboard models <span aria-hidden="true">→</span></span></motion.button>
          <motion.button className="categoryCard" onClick={() => navigate("spacers")} {...cardMotion(4)}><CategoryIcon type="spacer" /><strong>Spacers</strong><span className="projectCardDescription">Custom printable standoffs for precise gaps, fasteners and mounting surfaces.</span><span className="projectCardAction">Browse spacers <span aria-hidden="true">→</span></span></motion.button>
          <motion.button className="categoryCard" onClick={() => navigate("washers")} {...cardMotion(5)}><CategoryIcon type="washer" /><strong>Washers</strong><span className="projectCardDescription">Custom flat and countersunk washers for fasteners and mounting points.</span><span className="projectCardAction">Browse washers <span aria-hidden="true">→</span></span></motion.button>
          <motion.button className="categoryCard" onClick={() => navigate("mounting")} {...cardMotion(6)}><CategoryIcon type="mount" /><strong>Mounting</strong><span className="projectCardDescription">Brackets and finishing parts for clean, reliable hardware mounting.</span><span className="projectCardAction">Browse mounting parts <span aria-hidden="true">→</span></span></motion.button>
        </div>
      </motion.section>
    </> : view === "organizers" ? <>
