...
export function SimpleModelEditor({ spec }: { spec: SimpleModelSpec }) {
  ...
  return <main className="workbench projectsHome">
    ...
    <section className="workbenchIdentity" aria-label="Current model"><div><span>Model / configurable</span><h1>{spec.title}</h1></div><p>Changes are generated locally in your browser.</p></section>
    <section className="editor" aria-label={`${spec.title} editor`}>
      <aside className="parameters"><div className="parameterDetails"><div className="panelHeading"><span>Model settings</span><span className="unitLabel">mm</span></div>
        ...
        <div className="parameterStatus">{error ? <p className="error" role="alert">{error}</p> : <><p className="formHint">Updates apply to the preview automatically.</p>{model?.metadata.printability.messages.map((message) => <p className="formHint" key={message}>{message}</p>)}</>}</div></div>
      </aside>
